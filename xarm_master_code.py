...
camera_data = [0, 0, 0]
sensor_x = float(-113.7)
sensor_y = float(374.9)
pick_motor = [0 ,0, 0]
comp_x = 0
comp_y = 0
angle = 0.0
...
while True:
    ...
    if arm.get_cgpio_digital(0)[1] == 0 and params['variables'].get('move_robot', 0) == True:
        ...
        if arm.error_code == 0 and not params['quit']:
            code = arm.set_position(*[-68.3, 341.1, 434.4, -179.7, 1.2, 89.8], speed=params['speed'], mvacc=params['acc'], radius=-1.0, wait=True)
            if code != 0:
                params['quit'] = True
        ...
        print(pick_motor)
        if arm.error_code == 0 and not params['quit']:
            code = arm.set_position(*[pick_motor[0], pick_motor[1], 434, -180, 0, pick_motor[2]], speed=params['speed'], mvacc=params['acc'], radius=-1.0, wait=True)
            if code != 0:
                params['quit'] = True
                pprint('set_position, code={}'.format(code))
        if arm.error_code == 0 and not params['quit']:
